...
async function generateAndStore(examDate, env) {
  ...
  // ✅ FIX: Map for dedup — same phone keeps best score only
  ...
  for (const id in allData) {
    ...
    if (!r.examFile || !r.phoneNumber) continue;
    ...
    // ✅ DEDUP: same phone → best score மட்டும் வை
    const existing = byPaperMap[key].get(r.phoneNumber);
    if (existing) {
      dupSkipped++;
      if (entry.score > existing.score ||
         (entry.score === existing.score && entry.timeTaken < existing.timeTaken)) {
        byPaperMap[key].set(r.phoneNumber, entry);
      }
    } else {
      byPaperMap[key].set(r.phoneNumber, entry);
    }
